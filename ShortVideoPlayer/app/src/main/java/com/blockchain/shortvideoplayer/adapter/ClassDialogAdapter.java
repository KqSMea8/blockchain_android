...
public class ClassDialogAdapter extends BaseAdapter {
    ...
    private String TAG = ClassDialogAdapter.class.getSimpleName();
    private List<ClassModel> classModelList;
    private Context mContext;

    public ClassDialogAdapter(Context context, List<ClassModel> classModelList) {
        mContext = context;
    ...
    public View getView(final int pos, View view, ViewGroup arg2) {
        ...
        final ViewHolder vh;
        if (view == null) {
            view = LayoutInflater.from(mContext).inflate(R.layout.class_dialog_item, null);
            vh = new ViewHolder();
            vh.iv_thumbnail = view.findViewById(R.id.iv_thumbnail);
            vh.tv_name = view.findViewById(R.id.tv_name);
            vh.rl_image = view.findViewById(R.id.rl_image);
            view.setTag(vh);
        } else {
            vh = (ViewHolder) view.getTag();
        }

        final ClassModel classModel = classModelList.get(pos);
...
//            Picasso.with(mContext).load(classModel.getPic()).transform(new PicassoRoundTransform()).into(vh.iv_thumbnail);
        ...
        if(classModel.isSelected()){
            vh.rl_image.setBackgroundResource(R.drawable.blue_line_round_shape);
        }else {
            vh.rl_image.setBackground(null);
        }
        vh.rl_image.setOnClickListener(new View.OnClickListener() {
            @Override
