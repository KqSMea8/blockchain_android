...
import com.blockchain.shortvideoplayer.utils.CropSquareTransformation;
...
public class ClassFirstAdapter extends BaseAdapter {
    ...
    public ClassFirstAdapter(Context context, List<ClassFirstModel> classModelList) {
        mContext = context;
    ...
    public View getView(final int pos, View view, ViewGroup arg2) {
        ...
        ClassFirstModel classModel = classModelList.get(pos);

        if(!TextUtils.isEmpty(classModel.getPic())){
//            Picasso.with(mContext).load(classModel.getPic()).transform(new CropSquareTransformation()).fit().into(vh.iv_thumbnail);
