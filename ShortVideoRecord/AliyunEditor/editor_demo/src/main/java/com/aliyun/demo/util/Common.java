...
//            + "/";
    ...
    public static void copyAll(Context cxt, View view) {
        ...
        dir.mkdirs();
        Log.e("CopyFile", "bool" + dir.exists());
        unZip();
    }
            ...
            new AsyncTask() {
                ...
                protected Object doInBackground(Object[] objects) {
                    try {
                        ...
                        //判断解压后的文件是否存在,截取.zip之前的字符串,如果已经存在就不在解压不再添加到数据库中
                        List<FileDownloaderModel> list = DownloaderManager.getInstance().getDbController().getResourceByType(3);
                        if (!new File(file.getAbsolutePath().substring(0, len - 4)).exists() || list.size() <= 0) {
