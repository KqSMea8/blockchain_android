...
public class AliyunVideoCrop extends Activity implements TextureView.SurfaceTextureListener,
    ...
    private void startCrop() {
        ...
        try{
            ...
            Log.e(AliyunTag.TAG,"rotation" + rotation);
        }catch (Exception e) {
            Log.e(AliyunTag.TAG,"parse rotation failed");
        }
        nativeParser.release();
        nativeParser.dispose();
        Log.e(AliyunTag.TAG,"videoHeight:" + videoHeight + "videoWidth：" + videoWidth);

        float videoRatio = (float)videoHeight / videoWidth;
        Log.e(AliyunTag.TAG,"videoRatio:" + videoRatio);
        ...
        Log.e(AliyunTag.TAG,"outputRatio:" + (videoRatio>outputRatio));
        ...
        AliyunMediaExtractor mExtractor = new AliyunMediaExtractor();
        mExtractor.setDataSource(path);

        if ((rotation == 90 || rotation == 270)  && (mExtractor.getVideoRotation() != 90 && mExtractor.getVideoRotation() != 270)) {
            cropParam.setCropRect(new Rect(posY,posX,posY + cropWidth,posX+ cropHeight));
        } else {
            cropParam.setCropRect(new Rect(posX, posY, posX + cropWidth, posY + cropHeight));
        }
